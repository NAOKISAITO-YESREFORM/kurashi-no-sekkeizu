  ...
  const submitToFormspree = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubmitError("送信エラーが発生しました。お手数ですがお電話 (03-6671-1244) にてお問い合わせください。");
      }
    } catch (e) {
      setSubmitError("通信エラーが発生しました。電波の良い場所で再度お試しください。それでも続く場合はお電話 (03-6671-1244) にてご連絡ください。");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (step === summaryStep) {
    ...
    return (
      <Frame>
        ...
        {submitError && (
          <div style={{ background: "#FBE9E7", border: "1px solid #D84315", color: "#BF360C", padding: "12px 16px", marginBottom: 16, fontSize: 12, lineHeight: 1.7 }}>
            {submitError}
          </div>
        )}
